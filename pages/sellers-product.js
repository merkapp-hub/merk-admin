...
function SellersProduct(props) {
    ...
    }, [user]); // <- Re-run when searchText changes

    const getProduct = async () => {
        props.loader(true);

        let url = `getProduct?seller_id=${router?.query?.seller_id}`;
    ...
    const addNewItem = async () => {
        ...
        Api("post", 'updateProduct', data, router).then(
            (res) => {
                ...
                console.log("res================>", res);
                if (res.status) {
                    props.toaster({ type: "success", message: 'Item updated successfully' });
                    setPopupData({});
                    setviewPopup(false);
                    getProduct()
                }
    ...
    const bestSeller = ({ value, row }) => {
        return (
            <div className="p-4 flex items-center justify-center">
                <input className='h-5 w-5 text-black' type='checkbox' checked={selectedNewSeller.includes(row?.original?._id)}
                    onChange={(e) => {
                        console.log(e)
                        ...
                        if (!selectedNewSeller.includes(row?.original?._id)) {
                            ...
                            const newData = selectedNewSeller.filter(f => f !== row?.original?._id)
                            console.log(newData)
    ...
    const availableColor = ({ value }) => {
        return (
            <div className="p-4 flex  items-center justify-center gap-2">
                {value.map((item, i) => (<p key={i} className=" text-base font-normal  rounded-full h-5 w-5 border border-black" style={{ background: item?.color }}></p>))}
            </div>
        );
    };
    ...
    const actionHandler = ({ value, row }) => {
        return (
            <div className="bg-custom-offWhiteColor flex items-center  justify-evenly  border border-custom-offWhite rounded-[10px] mr-[10px]">
                <div className="py-[10px] w-[50%] items-center flex justify-center cursor-pointer"
                    onClick={() => { router.push(`add-product?id=${row.original._id}`) }}
                >
                ...
                <div className="py-[10px] border-l-[1px] border-custom-offWhite w-[50%] items-center flex justify-center">
                    <RiDeleteBinLine className="text-[red] text-[24px] cursor-pointer" onClick={() => deleteProduct(row.original._id)} />
                </div>
            </div>
        );
    };
    ...
    const actionHandler2 = ({ value, row }) => {
        return (
            <div className="bg-custom-offWhiteColor flex items-center  justify-evenly  border border-custom-offWhite rounded-[10px] mr-[10px]">
                ...
                <div className="py-[10px] border-l-[1px] border-custom-offWhite w-[50%] items-center flex justify-center">
                    <FaEye className="text-black text-[24px] cursor-pointer" onClick={() => { setPopupData(row.original); setviewPopup(true) }} />
                </div>
            </div>
        );
    };
    ...
    const suspendProduct = async (productId) => {
        try {
            ...
            Api("post", `suspend/${productId}`, null, router).then((res) => {
                ...
                if (res?.status) {
                    props.toaster({ type: "success", message: 'Item Suspended successfully' })
                    setviewPopup(false);
                } else {
                    console.log(res?.data?.message);
                    props.toaster({ type: "error", message: res?.data?.message });
            ...
            props.toaster({ type: "error", message: "An error occurred while suspending the product." });
        }
    };
    ...
    return (
        <div className=" w-full h-full bg-transparent pt-1 pb-5 pl-5 pr-5">
                    ...
                    <div className="relative w-[300px] md:w-[360px] h-auto  bg-white rounded-[15px] m-auto">
                        ...
                        <div className='px-5 py-10'>
                            <div className=' w-full flex gap-2 pb-5'>
                                <img src={popupData?.varients[0].image[0]} className='h-[76px] w-[76px] rounded-[10px]' />
                                <div className='col-span-2 w-full flex flex-col justify-start items-start'>
                                    <p className="text-base font-bold text-black">{popupData?.name}</p>
                                    <p className="text-base font-semibold text-black pt-2">{popupData?.email}</p>
                                    <p className="text-sm font-semibold text-black pt-2">{popupData?.number}</p>
                                </div>
                            </div>

                            <FormGroup>
                                <FormControlLabel control={<Checkbox checked={popupData?.sponsered} onChange={(e, sponsered) => { setPopupData({ ...popupData, sponsered }) }} />} label="is sponsered?" />
                                <FormControlLabel control={<Checkbox checked={popupData?.is_verified} onChange={(e, is_verified) => { setPopupData({ ...popupData, is_verified }) }} />} label="is verified?" />
                                <FormControlLabel control={<Checkbox checked={popupData?.is_quality} onChange={(e, is_quality) => { setPopupData({ ...popupData, is_quality }) }} />} label="is quality?" />
                            </FormGroup>

                            <div className='flex flex-row justify-start items-start pt-5 gap-5'>
                                <button className='text-white text-lg font-bold w-full h-[50px] rounded-[12px] bg-custom-darkpurple' onClick={() => { addNewItem() }}>Submit</button>
                                <button className='text-white text-lg font-bold w-full h-[50px] rounded-[12px] bg-custom-darkRed' onClick={() => suspendProduct(popupData?._id)} >Suspend</button>
                            </div>
            ...
            <div className='md:pt-[0px] pt-[0px] h-full'>
                <p className="text-black font-bold md:text-[32px] text-2xl">Seller Products</p>
                    ...
                    <div className="h-full">
                        {productsList?.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full w-full">
                                {/* <img src="/empty.png" alt="" className="w-[100px] h-[100px]" /> */}
                                <p className="text-black font-bold text-lg">No Product Found</p>
                            </div>
                        ) : <Table columns={columns1} data={productsList} />}
                        {/* <Table columns={user?.type === "SELLER" ? columns : columns1} data={productsList} /> */}
...
export default isAuth(SellersProduct)
